...
sudo apt-get install git python-pip mysql-server mysql-client \
     libmysqlclient-dev build-essential ipython libpng-dev libfreetype6-dev \
     libxft-dev libpython-dev liblapack-dev libblas-dev gfortran parallel

echo -e "Done installing aptitude packages."

echo -e "Making and installing"
...
python -c 'from mimclib.db import MIMCDatabase; print MIMCDatabase().DBCreationScript();' | mysql -u $USER
